...
| 檔案 | 內容 | 版控 | 是否評估過 |
|---|---|---|---|
| `questions.json` | **公開題庫（70 題）**：由真實使用者提問改寫而成 | **有** | ❌ **從未評估過** |
| `questions.private.json` | 直接取自 `logs/` 的原始題庫（50 題），未去識別化 | **無**（gitignore） | ✅ 2026-08-20 v4/v6/v7/v8 + judge |
| `questions.sample.json` | 早期的通用示例題（8 題），每類至少一題 | 有 | ❌ 未評分（僅 v8 冒煙一次） |

> ⚠ **現存所有評分報告都是跑在 `questions.private.json` 上的，而它不在 GitHub 上。**
> 公開題庫沿用同一批 id，但 50 題中有 **42 題的文字為去識別化而改寫**、
> 另新增 20 題，且分母改用 `expected_behavior` 判定 —— **數字不可直接並排**。
> 要在新環境建立基線，必須對 `questions.json` 重跑一次。
...
已提交的兩份報告摘要：
**[`report_20260820_judge.md`](report_20260820_judge.md)**（LLM judge 的 F/R/C 並排，
含 v8 未通過上線門檻的結論）與
**[`report_20260821_quant.md`](report_20260821_quant.md)**（免費的確定性百分比評分）。
