...
"""
claude_bible_rag_v7.py — Claude Sonnet 5 × FHL Bible Tools (agentic RAG engine)
===============================================================================
...
v7 (EXPERIMENTAL — NOT in production; v6 is the production engine):
  ⚠ Status (2026-08-20): kept in-repo for future development, NOT wired into
    server/chat.py. The 50-question × 3-engine evaluation (scripts/eval/)
    found the web search REGRESSED contemporary-question quality vs v6:
      * faithfulness 3.6 (v7) vs 4.4 (v6): the model misattributed or
        fabricated 信望愛站 article quotes — putting sentences in quotes and
        crediting them to articles whose fetched pages did not contain them
        (eval rows cont01/cont02/doct01).
      * coverage 3.9 (v7) vs 4.8 (v6).
      * cost: ~$0.045/question vs v6 ~$0.031, plus $10 per 1,000 searches.
    Before promoting v7, fix the prompt so article text is only quoted when
    verbatim in the search result, then re-run scripts/eval and confirm the
    contemporary category recovers. Everything below still applies.
  - Anthropic server-side web search (web_search_20250305 — see WEB_SEARCH_TOOL
    comment for why NOT the 20260209 dynamic-filtering variant), restricted to
    allowed_domains=["fhl.net"], for faith-vs-contemporary questions the
    Scripture/commentary tools cannot answer (e.g. 神有創造人嗎？怎麼解釋
    尼安德塔人的發現？ — the 信望愛站 article archive covers science &
    faith, but the sc.php/ssc.php commentary endpoints do not).
    Chosen over a DuckDuckGo scraper: DDG rate-limits this server's IP
    after 1-2 rapid requests (verified 2026-08-20), needs no extra
    dependency/key, and Claude reads + cites the pages server-side.
    Costs $10 per 1,000 searches on top of tokens; capped max_uses=3.
    Loop changes required by server tools:
      * server_tool_use / web_search_tool_result blocks appear inside the
        assistant message — nothing to dispatch client-side, but they are
        logged to the tool log ("🌐 web_search(...)").
      * stop_reason "pause_turn" (long server-tool turns) — resume by
        appending the paused assistant turn and continuing the loop;
        streamed text from paused rounds is carried into the final answer.
      * usage.server_tool_use.web_search_requests tracked in stats/usage_out
        (key "web_search"); server/chat.py adds it to the cost estimate.

v6 (claude_bible_rag_v6.py — kept as rollback):
...
# Anthropic server-side web search, restricted to the 信望愛站 domain family
# (allowed_domains covers subdomains: www/bible/a2z/ctb... .fhl.net). Runs on
# Anthropic's servers — no client-side dispatch, results arrive inside the
# assistant message. max_uses caps cost at 3 searches/query ($10 per 1,000).
# Kill switch without code changes: FHL_V7_WEB_SEARCH=0 in .env.
#
# DELIBERATELY the basic 20250305 variant, NOT web_search_20260209: the newer
# variant runs hidden code-execution "dynamic filtering" in a server container,
# which (a) requires threading response.container.id back on every follow-up
# request — miss it and the next round 400s with "container_id is required
# when there are pending tool uses generated by code execution" (production
# incident 2026-08-20 09:38: query crashed at round 3, turn lost, $0.17
# burned), and (b) let the model burn a 43s round experimenting inside the
# container. Plain search results are all this bot needs.
WEB_SEARCH_TOOL = {
    "type": "web_search_20250305",
    "name": "web_search",
    "max_uses": 3,
    "allowed_domains": ["fhl.net"],
}
WEB_SEARCH_ENABLED = os.environ.get("FHL_V7_WEB_SEARCH", "1") != "0"
...
## Web search — 信望愛站 articles

`web_search` is restricted to fhl.net (信望愛站). Use it for 當代議題 —
contemporary questions, new discoveries, or modern-day debates that the
Bible does not directly address in its own context (e.g. 演化論、尼安德塔人、
考古新發現、科技與生命倫理、女性主義、同志議題). For such topics, combine
BOTH relevant Scripture (existing tool chains) AND 信望愛站 articles found
via `web_search`. Typical flow: try `search_commentary` first; if it returns
nothing directly relevant, `web_search` for 信望愛站 articles.
- Search in 繁體中文 with short keywords (e.g. 「尼安德塔人 信仰」, 「同志 聖經」).
- Cite each article you use as a Markdown link: [文章標題](URL) — exact URL
  from the search result, never invented.
- Do NOT use web_search for purely biblical/doctrinal questions answerable
  from Scripture, commentary, or word-study tools alone (e.g. 因信稱義、
  某段經文的解釋) — those stay on the existing tool chains.
...
- **Web article citations:** whenever you quote or summarize a fhl.net
  article found via `web_search`, name it as a Markdown link —
  [文章標題](URL), URL copied exactly from the search result. Never mention
  an article title without its link.
...
def bible_query(
    ...
    if WEB_SEARCH_ENABLED:
        # Appended last so the client-tool prefix stays byte-identical for
        # prompt caching whether or not a subset of FHL tools is active.
    ...
    def _track(response):
        ...
        if server_use is not None:
            stats["web_search"] += getattr(server_use, "web_search_requests", 0) or 0
        if usage_out is not None:
            usage_out["model"] = MODEL_ID
            usage_out.update(stats)

    def _log_summary(rounds: int):
        searches = f" | web searches {stats['web_search']}" if stats["web_search"] else ""
        _log(f"[Summary] {rounds} round(s) | Claude {stats['claude_s']:.1f}s | "
             ...
             f"uncached {stats['uncached_in']} tok in{searches}")
    ...
    # Text streamed during pause_turn rounds (long server-tool turns split
    # across responses) — carried into the final answer so the persisted turn
    # matches what the client already saw streamed.
    carried_text: list[str] = []

    def _log_server_tool_blocks(round_num: int, content) -> None:
        """Log server-side web_search activity (already executed by Anthropic)."""
        for b in content:
            btype = getattr(b, "type", None)
            if btype == "server_tool_use":
                args_str = json.dumps(dict(b.input), ensure_ascii=False)
                _log(f"[Round {round_num}] 🌐 {b.name}({args_str})")
            elif btype == "web_search_tool_result":
                c = b.content
                if isinstance(c, list):
                    titles = "; ".join(getattr(r, "title", "")[:60] for r in c[:3])
                    _log(f"  → {len(c)} result(s): {titles}")
                else:  # error object, not a list
                    _log(f"  → web_search error: {getattr(c, 'error_code', c)}")
    ...
    for round_num in range(1, MAX_TOOL_ROUNDS + 1):
        ...
        _track(response)
        _log_server_tool_blocks(round_num, response.content)

        # Server-tool container (code-execution based tools): thread the id
        # back on follow-up requests or they 400. Never set with the basic
        # web_search_20250305 variant — kept for safety if the tool type is
        # ever upgraded to a container-backed variant again.
        container = getattr(response, "container", None)
        if container is not None and getattr(container, "id", None):
            api_kwargs["container"] = container.id

        round_text = ("".join(accumulated) if stream_callback
                      else "\n".join(b.text for b in response.content if b.type == "text"))
        tool_use_blocks = [b for b in response.content if b.type == "tool_use"]

        # pause_turn: a long server-tool turn was split — resend the paused
        # assistant turn as-is and let Claude continue it. Also the safety net
        # for stop_reason "tool_use" with only server-side blocks (nothing to
        # dispatch client-side).
        if response.stop_reason == "pause_turn" or (
            response.stop_reason == "tool_use" and not tool_use_blocks
        ):
            _log(f"[Round {round_num}] ⏸ Turn paused — resuming ({elapsed:.1f}s)")
            if round_text:
                carried_text.append(round_text)
            messages.append({"role": "assistant", "content": response.content})
            continue

        if response.stop_reason != "tool_use":
            full_text = "".join(carried_text) + round_text
    ...
    if stream_callback:
        ...
        return _postprocess_answer("".join(carried_text) + "".join(accumulated))
